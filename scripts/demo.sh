...
# demo hits the live application from the outside, the same way the reviewer will
...
# US-01: sample pipeline returns counts
run_story "US-01" \
...
# US-02: graph stats endpoint returns ints
run_story "US-02" \
...
# US-03: happy-path query returns answer + citations
run_story "US-03" \
...
# US-04 [ERROR PATH]: empty input -> 400 with structured error
run_story "US-04" \
  "Empty input returns 400 with 'input text is required'" \
  "code=\$(curl -s -o /tmp/_us04.json -w '%{http_code}' -X POST $APP_URL/api/query \
        -H 'Content-Type: application/json' -d '{\"text\":\"\"}'); \
   test \"\$code\" = '400' && grep -q 'input text is required' /tmp/_us04.json"

# US-05 [ERROR PATH]: missing key -> 503. Skipped unless QUERY_LLM_API_KEY is unset.
if [[ -z "${QUERY_LLM_API_KEY:-}" ]]; then
  run_story "US-05" \
    "Missing LLM credential returns 503 with operator-readable error" \
    "code=\$(curl -s -o /tmp/_us05.json -w '%{http_code}' -X POST $APP_URL/api/query \
          -H 'Content-Type: application/json' -d '{\"text\":\"hello\"}'); \
     test \"\$code\" = '503' && grep -q 'not configured' /tmp/_us05.json"
else
  echo
  echo "--- US-05: skipped (QUERY_LLM_API_KEY is set; unset it to test the 503 path) ---"
fi
