...
# Prevent namespace-package fallback into the real src/myproject/ tree during
# regeneration tests. If the model did not emit __init__.py, provide a tiny
# stub so imports resolve strictly from regenerated/src/myproject first.
if [[ ! -f "$OUTPUT_DIR/src/myproject/__init__.py" ]]; then
  cat > "$OUTPUT_DIR/src/myproject/__init__.py" <<'PYEOF'
__version__ = "0.1.0"
PYEOF
fi
